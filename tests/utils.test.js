...
describe('util tests', function() { // eslint-disable-line
  ...
  it('isLocalModule returns correct result.', () => {
    utils.setProjectRoot('/app');
    utils.setPkgJson({
      babel: {
        plugins: [
          [
  ...
  it('resolveModulePath returns correct path', () => {
    utils.setProjectRoot('/app');
    utils.setPkgJson({
      babel: {
        plugins: [
    ...
    const file = '/app/src/features/f-1/Test.js';
    ...
    expect(p1).to.equal('/app/src/features/f-2/redux/actions');
    expect(p2).to.equal('/app/src/common/routeConfig');
    expect(p3).to.equal('/app/src/features/f-1/M1');
    expect(p4).to.equal('react');
    expect(p5).to.equal('/app/src/features/f-3/index');
  });
});
